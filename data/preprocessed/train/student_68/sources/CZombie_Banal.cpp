...
CZombie_Banal::~CZombie_Banal()
{
	CLog::getInstance().printLog("S-a apelat destructorul clasei CZombie_Banal \n");
}

void CZombie_Banal::Display(string type_Zombie, int x, float y, CLevel& level)
{
	...
	if (health==NULL)
	{
		...
		fprintf(health, "%d ", 20); //HEALTH SUPER ZOMBIE=20. DACA SE DORESTE SCHIMBARE, SE VA MODIFICA SI AICI !!
	}
	else
	{
		int index = 0;
	...
	float last_yy_BeforeShot = 0;
	int last_xx_BeforeShot = 0;
			...
			if (a - 1 == BEGIN_LOGIN_BOX_COL + (END_LOGIN_BOX_COL - BEGIN_LOGIN_BOX_COL) / 10 || a == 0 || a == 1)// || a=0 : lovit de un frozenpea, a=1 lovit de un pea
			{
				...
				if (much <= 0 || a - 1 == BEGIN_LOGIN_BOX_COL + (END_LOGIN_BOX_COL - BEGIN_LOGIN_BOX_COL) / 10)
				{
					...
					while (fscanf(fZombie, "%d %f", &last_x, &last_y) > 0)
					{
						...
						{
							last_xx_BeforeShot = last_x;
							last_yy_BeforeShot = last_y;
						}

					}
					fclose(fZombie);
					disponibleLineNewZombie = 1;
					conOut(last_yy_BeforeShot, last_xx_BeforeShot) << "     ";
				...
				else
				{
					if (a == 0 || a == 1)
					{
						fZombie = fopen(Filenamech, "w");
						fprintf(fZombie, "%s\n", "ZB");
						fprintf(fZombie, "%d\t%.2f\n", XbeforeShot, YbeforeShot); //posibile erori. last_xx, last_yy pastreaza tot xbeforeshot,ybeforeshot
						a = XbeforeShot;
						b = YbeforeShot;
						fclose(fZombie);
					}
