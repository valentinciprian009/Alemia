...
void CLevel1::DrawLand()
{
	...
	for (float i = float(BEGIN_LOGIN_BOX_ROW + float(END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00); i <= float(END_LOGIN_BOX_ROW- float(END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00); i = i+ float((END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00))
	{
		for (int j = 0; j < 12; j++)
		{
			...
			if (i == BEGIN_LOGIN_BOX_ROW + float(3 * (END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00) || i == BEGIN_LOGIN_BOX_ROW + float(4 * (END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00) || i == float( BEGIN_LOGIN_BOX_ROW + float (END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00))
			{
				for (int k = BEGIN_LOGIN_BOX_COL + 1; k <= END_LOGIN_BOX_COL - 1; k++)
					conOut(i, k) << printedChar;
				for (int k = float((4 * (END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00)); k <= float(5 * (END_LOGIN_BOX_ROW - BEGIN_LOGIN_BOX_ROW) / 6.00);k++)
					conOut(k-1,END_LOGIN_BOX_COL)<<" ";
			}
		}
	}
	char printedChar2 = 186;
	char printedChar3 = 216;
