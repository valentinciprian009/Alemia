...
int x_, Appearance_ = 0;
float y_;


CZombie_Super::~CZombie_Super()
{
	CLog::getInstance().printLog("S-a apelat destructorul clasei CZombie_Super \n");
}

void CZombie_Super::Display(string type_Zombie, int x, float y, CLevel& level)
{
	...
	int much = 9990, u; //health

	string statusPVSpower = Filename2 + "PS";
	statusPVSpower += ".txt";
	...
	if (fZombie)
	{
		...
		{
			if (strcmp(typeZombie, typeZ) != 0 )// fscanf(fZombie, "%d", &u)>0) //in cazul in care 2 zombie tip Super sunt pe 2 linii diferite si se apeleaza display de coordonate incucisate
			{
				fclose(fZombie);
				return;
			}
	...
	{
		if (strcmp(typeZombie, "ZS") != 0)
			return;
	}


	int status = 0, time_move = 900;//viteza usor marita fata de a celorlalti

	FILE* SnowPAttack = fopen(FileAttackCh, "r");
	if (SnowPAttack)
	{
		if (fscanf(SnowPAttack, "%d", &status) > 0) //viteza scazuta dupa atac de frozeen pea
			time_move = 17000;
		fclose(SnowPAttack);
	}
