...
void Body::move(double time)    //Move the body object
{
	...
	double x_value = distance * cos(direction * (PI / 180));
	...
	double y_value = distance * sin(direction * (PI / 180));
...
{
	rand(); //Throw out first set of random numbers
	//Get a random decimal number between 0 and 1
	double random_number = rand() / (static_cast<double>(RAND_MAX) + 1);
