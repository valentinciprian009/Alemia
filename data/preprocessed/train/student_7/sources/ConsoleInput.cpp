...
void UserInterface::ConsoleInput::setGameMode()
{
	// Get the standard input handle. 
	hStdin = GetStdHandle(STD_INPUT_HANDLE);
	if (hStdin == INVALID_HANDLE_VALUE)
...
std::string ConsoleInput::mouseEventProc(MOUSE_EVENT_RECORD ir)
{
	...
	mousePos.first = ir.dwMousePosition.X;
	mousePos.second = ir.dwMousePosition.Y;
	return output;
}
...
std::pair<int, int> UserInterface::ConsoleInput::getMousePos()
{
	return mousePos;
}

bool UserInterface::ConsoleInput::getHasDoubleClicked()
{
