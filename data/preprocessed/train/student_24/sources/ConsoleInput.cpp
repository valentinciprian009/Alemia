...
void ConsoleInput::getCoordinates(int& x, int& y, std::string output)
{
	if (output != "")
	{
		std::string delim = ", ";
		size_t poz = output.find(delim);
		y = std::atoi(output.substr(0, poz).c_str());
		output.erase(0, poz + delim.length());
		x = std::atoi(output.c_str());
	}
}