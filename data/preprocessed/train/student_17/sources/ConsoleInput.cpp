	...
	if (nr)
	{
		...
		for (i = 0; i < cNumRead; i++)
		{
			...
			case KEY_EVENT: // keyboard input 
				return keyEventProc(irInBuf[i].Event.KeyEvent);//asta nu face mare lucru

			case MOUSE_EVENT: // mouse input 
				return mouseEventProc(irInBuf[i].Event.MouseEvent);//doar asta e tratata mai sus

			case WINDOW_BUFFER_SIZE_EVENT: // scrn buf. resizing 
				return resizeEventProc(irInBuf[i].Event.WindowBufferSizeEvent);//nici asta
