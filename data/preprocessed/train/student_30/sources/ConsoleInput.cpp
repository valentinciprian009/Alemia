...
std::pair<int, int> UserInterface::ConsoleInput::mouseEventPositions()
{
	DWORD nr = 0;
	...
	if (nr)
	{
		...
		for (i = 0; i < cNumRead; i++)
			if (irInBuf[i].EventType == MOUSE_EVENT) {
				auto ev = irInBuf[i].Event.MouseEvent;
				if (ev.dwEventFlags == DOUBLE_CLICK) {
					position.first = ev.dwMousePosition.Y;
					position.second = ev.dwMousePosition.X;
				}
