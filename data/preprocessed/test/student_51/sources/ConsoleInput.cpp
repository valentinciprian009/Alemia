...
std::string ConsoleInput::mouseEventProc(MOUSE_EVENT_RECORD ir)
{
	...
	case DOUBLE_CLICK:
		...
		output += std::to_string(ir.dwMousePosition.Y);
		output += ")";
		coord.x = atoi(std::to_string(ir.dwMousePosition.X).c_str());
		coord.y = atoi(std::to_string(ir.dwMousePosition.Y).c_str());


		return output;

	//case 0x0000:
	//	output += "mouse simple clicked at (";
	//	output += std::to_string(ir.dwMousePosition.X);
	//	output += ", ";
	//	output += std::to_string(ir.dwMousePosition.Y);
	//	output += ")";
