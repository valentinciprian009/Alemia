...
std::string ConsoleInput::mouseEventProc(MOUSE_EVENT_RECORD ir,int *&coord)
{
	...
	case MOUSE_MOVED:	
	{
		if (GetAsyncKeyState(VK_LBUTTON)) {
			...
			coord[0] = ir.dwMousePosition.X;
			coord[1] = ir.dwMousePosition.Y;
		}
		else
		{
...
std::string UserInterface::ConsoleInput::getUserActions(int *&coord)
{
	DWORD nr = 0;
	...
	if (nr)
	{
		...
		for (i = 0; i < cNumRead; i++)
		{
			...
			case MOUSE_EVENT: // mouse input 
				return mouseEventProc(irInBuf[i].Event.MouseEvent,coord);
