...
UserAction ConsoleInput::mouseEventProc(MOUSE_EVENT_RECORD ir)
{
	...
	case 0:
		action.SetOperation(Operation::Click);
		action.AddParam(std::to_string(ir.dwMousePosition.X));
		action.AddParam(std::to_string(ir.dwMousePosition.Y));     
		return action;
	}
	return action;
}
...
UserAction UserInterface::ConsoleInput::getUserActions()
{
	DWORD nr = 0;
	UserAction action;
