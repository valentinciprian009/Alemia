...
ConsoleInput* ConsoleInput::mpInstance = NULL;

ConsoleInput& ConsoleInput::getInstance()
{
	if (mpInstance == NULL)
	{
		mpInstance = new ConsoleInput();
	}
	return *mpInstance;
}

void UserInterface::ConsoleInput::set_x(int x)
{
	this->x = x;
}

void UserInterface::ConsoleInput::set_y(int y)
{
	this->y = y;
}

int UserInterface::ConsoleInput::get_x()
{
	return x;
}

int UserInterface::ConsoleInput::get_y()
{
	return y;
}
...
std::string ConsoleInput::mouseEventProc(MOUSE_EVENT_RECORD ir)
{
	...
	case DOUBLE_CLICK:
		...
		output += std::to_string(ir.dwMousePosition.Y);
		output += ")              ";
		set_x(ir.dwMousePosition.X);
		set_y(ir.dwMousePosition.Y);
		return output;
	}
	return output;
}
